...
import { FiHome, FiPlusCircle, FiList, FiBell } from "react-icons/fi";
...
const OrganizerNavbar = () => {
  ...
  const navLinks = [
    { path: "/organizer/dashboard", label: "Dashboard", icon: <FiHome className="text-lg" /> },
    { path: "/organizer/create-event", label: "Create Event", icon: <FiPlusCircle className="text-lg" /> },
    { path: "/organizer/my-events", label: "My Events", icon: <FiList className="text-lg" /> },
  ];
        ...
        <div className="flex items-center gap-8">
          <Link to="/organizer/dashboard" className="flex items-center gap-2 group">
            ...
            <div className="flex flex-col items-start leading-none">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-wider text-primary opacity-80">Organizer</span>
            </div>
          </Link>
               ...
               <Link
                 ...
                 className={`text-sm font-medium transition-all flex items-center gap-2 px-3 py-2 rounded-lg 
                   ${isActive(link.path)
                     ...
                     : "text-gray-500 dark:text-slate-400 hover:text-primary dark:hover:text-blue-400 hover:bg-gray-50 dark:hover:bg-slate-800"
                   }`}
               >
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden lg:block bg-gray-50 dark:bg-slate-800 rounded-xl p-1 border border-gray-100 dark:border-slate-700">
            <div className="flex items-center px-3 gap-2">
              <span 
                className="material-symbols-outlined text-gray-400 dark:text-slate-500 text-xl cursor-pointer hover:text-primary dark:hover:text-blue-400 transition-colors"
                onClick={() => {
                  const val = document.getElementById("navbar-search")?.value?.trim();
                  if (val !== undefined) navigate(`/organizer/my-events?search=${encodeURIComponent(val)}`);
                }}
              >
                search
              </span>
              <input 
                ...
                key={location.search} // re-renders on route/search change to stay synced
                onKeyDown={(e) => { 
                  if (e.key === "Enter") {
                    const val = e.target.value.trim();
                    navigate(`/organizer/my-events${val ? `?search=${encodeURIComponent(val)}` : ''}`);
                  }
                }}
              />
            </div>
          ...
          <Link to="/notifications" className="relative group p-2 hover:bg-blue-50 dark:hover:bg-slate-800 rounded-xl transition-all mr-1">
             <FiBell className="text-xl text-gray-400 dark:text-slate-500 group-hover:text-primary dark:group-hover:text-blue-400" />
             {unreadCount > 0 && (
          ...
          <Link to="/profile" className="w-9 h-9 rounded-full bg-gray-50 dark:bg-slate-800 flex items-center justify-center border border-gray-200 dark:border-slate-700 cursor-pointer overflow-hidden font-bold text-primary dark:text-blue-400 hover:border-primary transition-all shadow-sm" title="My Profile">
             {user?.firstName?.[0] || "O"}
          </Link>

          <button className="lg:hidden text-gray-500 dark:text-slate-400 hover:text-primary dark:hover:text-blue-400 transition-colors" onClick={() => setMobileOpen(!mobileOpen)}>
