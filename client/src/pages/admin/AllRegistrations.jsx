...
import { FiUsers, FiMail, FiPhone, FiArrowLeft, FiSearch, FiDownload, FiUserCheck, FiTag } from "react-icons/fi";
...
const AllRegistrations = () => {
  ...
  const filteredRegistrations = registrations.filter(a => {
    const matchesSearch = 
      ...
      (a.event?.organizer && `${a.event.organizer.firstName} ${a.event.organizer.lastName}`.toLowerCase().includes(search.toLowerCase()));
  ...
  const exportToCSV = () => {
    ...
    const headers = ["Event", "Organizer", "Student Name", "Email", "Phone", "Enrollment No", "Department", "Year", "Status", "Payment Status", "Registration Date"];
    
    const csvContent = [
      headers.join(","),
      ...filteredRegistrations.map(a => [
        `"${a.event?.title || 'Unknown Event'}"`,
        `"${a.event?.organizer ? `${a.event.organizer.firstName} ${a.event.organizer.lastName}` : 'Unknown'}"`,
    ...
    link.setAttribute("download", `All_Registrations_Platform_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <main className="flex-grow max-w-[1240px] w-full mx-auto px-4 sm:px-8 py-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            ...
            <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">System <span className="text-primary dark:text-blue-400 italic">Registrations</span></h2>
            <p className="text-gray-500 dark:text-slate-400 font-medium text-sm mt-1">Platform-wide overview of all student event registrations.</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col lg:flex-row gap-4 mb-8 transition-colors">
           <div className="relative flex-grow group">
              ...
              <input type="text" placeholder="Search by student, email, event, or organizer..." value={search} onChange={(e) => setSearch(e.target.value)}
        ...
        {loading ? (
          <div className="flex flex-col items-center py-24">
             ...
             <p className="text-gray-400 dark:text-slate-500 font-bold uppercase tracking-widest text-[9px]">Loading Records...</p>
          </div>
          ...
          <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm transition-colors">
             <p className="text-gray-400 dark:text-slate-500 font-bold">No registrations found.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50/50 dark:bg-slate-800/50 text-[10px] uppercase tracking-widest text-gray-400 dark:text-slate-500 font-black">
                    <th className="px-8 py-5">Event & Organizer</th>
                ...
                <tbody className="divide-y divide-gray-50 dark:divide-slate-800/50">
                   <AnimatePresence>
                     {filteredRegistrations.map((a, idx) => (
                       <motion.tr key={a._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: Math.min(idx * 0.03, 1.5) }} className="hover:bg-gray-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                         <td className="px-8 py-5 w-[25%]">
                            ...
                            <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 mt-1 uppercase tracking-widest flex items-center gap-1">
                               <FiTag className="text-primary dark:text-blue-400" /> {a.event?.organizer ? `${a.event.organizer.firstName} ${a.event.organizer.lastName}` : "Unknown"}
                            </p>
                         </td>
                         ...
                         <td className="px-8 py-5">
                           ...
                           {a.paymentStatus && a.paymentStatus !== "not_required" && (
                             <span className={`block mt-2 px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest w-fit border ${
                               a.paymentStatus === 'approved' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-900/30' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-900/30'
                             }`}>
